...
**AUTOLOADED_CONTEXT_MODELS** lists which context models automatically process incoming JSON-LD documents. When `LinkedDataDocument.load()` parses a document, it walks through these models and calls `load_from_graph()` on each.

The default list includes all ActivityStreams 2.0 and Security Vocabulary context models:
...
Message processors intercept and transform documents before they're processed (incoming) or delivered (outgoing). **MESSAGE_PROCESSORS** lists processor class paths:
...
`ActorDeletionMessageProcessor` drops Delete activities for actors your instance has never seen. Mastodon broadcasts deletion activities widely, including to servers that never interacted with the deleted actor. This processor prevents wasting resources processing irrelevant deletions.

`CompactJsonLdMessageProcessor` strips namespace prefixes from outgoing documents. Some Fediverse servers don't properly handle JSON-LD and expect unprefixed attribute names. This processor ensures compatibility by transforming `as:name` to `name` in outgoing messages.
...
A processor can raise `DropMessage` to abort processing entirely. This prevents the document from being stored or parsed. Use this for messages that fail validation or policy checks.
...
Settings are organized in nested classes (`Instance`, `NodeInfo`, `LinkedData`, etc.) for clarity. Some properties like `AUTOLOADED_CONTEXT_MODELS` perform lazy import of string paths to actual classes.
